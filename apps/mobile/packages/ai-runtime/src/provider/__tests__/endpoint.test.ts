...
describe('resolveWireModelId', () => {
  ...
  it('strips the Gemini listing prefix on the google endpoint', () => {
    ...
    expect(resolveWireModelId(model, ENDPOINT_TYPE.GOOGLE_GENERATE_CONTENT)).toBe(
      'gemini-flash-latest',
    );
  });
...
describe('resolveAiSdkProviderId', () => {
  describe('Catalog adapterFamily (highest priority)', () => {
    it('uses adapterFamily on the selected endpoint, overriding provider.id heuristics', () => {
      ...
      expect(resolveAiSdkProviderId(provider, ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS)).toBe(
        'openai-compatible',
      );
    ...
    it('passes already-variant adapterFamily through idempotently', () => {
      ...
      expect(resolveAiSdkProviderId(provider, ENDPOINT_TYPE.OPENAI_RESPONSES)).toBe(
        'azure-responses',
      );
    });
    ...
    it('returns openai-compatible when adapterFamily is unknown', () => {
      ...
      expect(resolveAiSdkProviderId(provider, ENDPOINT_TYPE.ANTHROPIC_MESSAGES)).toBe(
        'openai-compatible',
      );
    });
  ...
  describe('Relay-style multi-endpoint provider (post-migration shape)', () => {
    ...
    it('routes anthropic-messages endpoint to the anthropic adapter (REGRESSION)', () => {
      ...
      expect(resolveAiSdkProviderId(makeMiniMaxLike(), ENDPOINT_TYPE.ANTHROPIC_MESSAGES)).toBe(
        'anthropic',
      );
    });

    it('routes openai-chat-completions endpoint to openai-compatible adapter (REGRESSION)', () => {
      expect(resolveAiSdkProviderId(makeMiniMaxLike(), ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS)).toBe(
        'openai-compatible',
      );
    });
...
describe('resolveProviderVariant', () => {
  it('appends -chat for openai-chat-completions on bases with a chat variant', () => {
    expect(resolveProviderVariant('openai' as never, ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS)).toBe(
      'openai-chat',
    );
  });
  ...
  it('returns the base id for openai-responses on bases without a -responses variant', () => {
    ...
    expect(resolveProviderVariant('openai' as never, ENDPOINT_TYPE.OPENAI_RESPONSES)).toBe(
      'openai',
    );
  });

  it('appends -responses for azure base (real variant)', () => {
    expect(resolveProviderVariant('azure' as never, ENDPOINT_TYPE.OPENAI_RESPONSES)).toBe(
      'azure-responses',
    );
  });

  it('returns the base id unchanged when no variant is registered', () => {
    expect(resolveProviderVariant('deepseek' as never, ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS)).toBe(
      'deepseek',
    );
  });
...
describe('resolveEffectiveEndpoint', () => {
  ...
  describe('multi-backend gateway per-model routing (AiHubMix)', () => {
    ...
    it.each([
      ...
      'resolves %s → %s / %s from the model id',
      (id, endpointType, providerOptionsKey) => {
    ...
    it('routes by apiModelId when present (renamed/user-added ids)', () => {
      ...
      expect(resolveEffectiveEndpoint(aihubmix, model).endpointType).toBe(
        ENDPOINT_TYPE.ANTHROPIC_MESSAGES,
      );
    });
    ...
    it('does NOT route to an endpoint the provider row does not declare (stale insert-only seed)', () => {
      ...
      expect(
        resolveEffectiveEndpoint(staleAihubmix, { id: 'gemini-2.5-pro' } as never).endpointType,
      ).toBe(ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS);
      expect(
        resolveEffectiveEndpoint(staleAihubmix, { id: 'gemini-2.5-pro' } as never)
          .providerOptionsKey,
      ).toBeUndefined();
      ...
      expect(
        resolveEffectiveEndpoint(staleAihubmix, { id: 'claude-opus-4-6' } as never).endpointType,
  ...
  describe('multi-backend gateway per-model routing (DMXAPI)', () => {
    ...
    it('keeps a stale row without the Google endpoint on its existing chat route', () => {
      ...
      expect(
        resolveEffectiveEndpoint(staleDmxapi, { id: 'gemini-2.5-pro' } as never).endpointType,
      ).toBe(ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS);
      expect(
        resolveEffectiveEndpoint(staleDmxapi, { id: 'claude-opus-4-6' } as never).endpointType,
      ).toBe(ENDPOINT_TYPE.ANTHROPIC_MESSAGES);
    });
  });
});
