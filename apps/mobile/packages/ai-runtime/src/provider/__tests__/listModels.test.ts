...
import { ENDPOINT_TYPE, MODEL_CAPABILITY } from '@cherrystudio/universal/data/types/model';
...
const { listModels: listPortableModels } = await import('../listModels');

function listModels(
  provider: Parameters<typeof listPortableModels>[0],
  signal?: AbortSignal,
  ...
  return listPortableModels(
    provider,
    {
      appHeaders: { 'User-Agent': 'CherryStudio-Test' },
      getAuthConfig: getAuthConfigMock,
      getCopilotToken: async (headers) => (await getCopilotTokenMock(null, headers)).token,
      getRotatedApiKey: getRotatedApiKeyMock,
      getVertexAuthHeaders: getAuthHeadersMock,
    },
    signal,
    options,
  );
...
describe('listModels — geminiFetcher API key transport', () => {
  it('passes the API key via the x-goog-api-key header, never the ?key= query (REGRESSION)', async () => {
    ...
    const call = aiSdkGetFromApiMock.mock.calls[0][0] as {
      url: string;
...
describe('listModels — openRouterFetcher image models', () => {
  ...
  it('unions the dedicated image catalog and marks duplicate image models for image routing', async () => {
    ...
    aiSdkGetFromApiMock.mockImplementation(({ url }: { url: string }) => {
      ...
      return Promise.resolve({
        value: { data: [{ id: 'anthropic/claude-sonnet-4' }, { id: 'openai/gpt-image-2' }] },
      });
    });
    ...
    expect(
      models.find((model) => model.apiModelId === 'sourceful/riverflow-v2.5-fast'),
    ).toMatchObject({
      capabilities: [MODEL_CAPABILITY.IMAGE_GENERATION],
      endpointTypes: [ENDPOINT_TYPE.OPENAI_IMAGE_GENERATION],
...
describe('listModels — Radeon Cloud source header', () => {
  it('adds X-Source to Radeon model listing without adding it to other providers', async () => {
    ...
    const radeonCall = aiSdkGetFromApiMock.mock.calls[0][0] as {
      url: string;
      headers: Record<string, string>;
    };
    const otherCall = aiSdkGetFromApiMock.mock.calls[1][0] as {
      url: string;
      headers: Record<string, string>;
    };
...
describe('listModels — vertexFetcher (per-publisher pagination)', () => {
  ...
  it('queries every default publisher under the location aiplatform host with the signed headers', async () => {
    ...
    for (const publisher of DEFAULT_VERTEX_MODEL_PUBLISHERS) {
      expect(
        urls.some((u) =>
          u.startsWith(
            `https://us-central1-aiplatform.googleapis.com/v1beta1/publishers/${publisher}/models?`,
          ),
        ),
      ).toBe(true);
    }
    const firstHeaders = (
      aiSdkGetFromApiMock.mock.calls[0][0] as { headers: Record<string, string> }
    ).headers;
  ...
  it('preserves the publisher prefix for MaaS models published by Google', async () => {
    aiSdkGetFromApiMock.mockResolvedValue({
      value: {
        publisherModels: [
          { name: 'publishers/google/models/gemma-4-26b-a4b-it-maas', displayName: 'Gemma 4 26B' },
        ],
      },
    });
