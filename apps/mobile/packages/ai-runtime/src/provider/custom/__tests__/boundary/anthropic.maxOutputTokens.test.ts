...
function captureRequest(
  ...
  return captureWithFetch((fetch) =>
    modelFactory(fetch).doStream({ prompt, ...options } as LanguageModelV3CallOptions),
  );
}
...
describe('@ai-sdk/anthropic maxOutputTokens boundary', () => {
  it('omits max_tokens for an unknown public Anthropic model when no limit is supplied', async () => {
    const request = await captureRequest((fetch) =>
      createAnthropic({ apiKey: 'sk', baseURL: 'https://example.com/v1', fetch }).languageModel(
        'minimax-m2.5',
      ),
    );
