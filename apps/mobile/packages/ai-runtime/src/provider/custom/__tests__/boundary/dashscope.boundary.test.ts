  ...
  },
  {
    ...
    schema: z.strictObject({
      ...
      input: z.strictObject({
        messages: z.array(
          z.strictObject({ role: z.literal('user'), content: z.array(messagePart) }),
        ),
      }),
    }),
  },
...
describe('DashScope poll resume (restart-safe response family)', () => {
  const succeeded = (output: Record<string, unknown>) =>
    new Response(JSON.stringify({ output: { task_status: 'SUCCEEDED', ...output } }), {
      status: 200,
    });
