...
describe('AihubmixImageModel', () => {
  ...
  describe('Google native image models', () => {
    it('routes Gemini image models through @ai-sdk/google and normalizes image config', async () => {
      ...
      const result = await model.doGenerate(
        callOptions({
          providerOptions: {
            aihubmix: { mode: 'generate', aspectRatio: 'ASPECT_16_9', imageSize: '2k' },
          } as any,
        }),
      );
    ...
    it('routes Imagen models through @ai-sdk/google predict and normalizes personGeneration', async () => {
      const fetchMock = vi
        .fn()
        .mockResolvedValue(okJson({ predictions: [{ bytesBase64Encoded: 'IMG' }] }));
      vi.stubGlobal('fetch', fetchMock);
      ...
      const [url, init] = fetchMock.mock.calls[0];
      expect(url).toBe(
        'https://aihubmix.com/gemini/v1beta/models/imagen-4.0-generate-preview-06-06:predict',
      );
      const body = JSON.parse(init.body as string);
        ...
        callOptions({
          providerOptions: {
            aihubmix: {
              ...
              imageWeight: 55,
              imageFiles: [
                { mediaType: 'image/png', data: new Uint8Array([1, 2]), name: 'src.png' },
              ],
            },
          } as any,
        }),
  ...
  describe('Ideogram V_1/V_2', () => {
    it('generate → JSON {image_request} to /ideogram/aihubmix_image_generate', async () => {
      const fetchMock = vi
        .fn()
        .mockResolvedValue(okJson({ data: [{ url: 'https://img/v1.png' }] }));
      vi.stubGlobal('fetch', fetchMock);
  ...
  describe('response parsing', () => {
    it('parses data.output.b64_json[].bytesBase64', async () => {
      vi.stubGlobal(
        ...
        vi
          .fn()
          .mockResolvedValue(
            okJson({ output: { b64_json: [{ bytesBase64: 'OUT1' }, { bytesBase64: 'OUT2' }] } }),
          ),
      );
    ...
    it('parses data.data[].url verbatim', async () => {
      vi.stubGlobal(
        'fetch',
        vi.fn().mockResolvedValue(okJson({ data: [{ url: 'https://img/x.png' }] })),
      );
      const result = await make('V_2').doGenerate(callOptions());
  ...
  describe('default branch delegates byte-identically to the inner OpenAICompatibleImageModel', () => {
    for (const id of ['gpt-image-1', 'gpt-image-2', 'FLUX.1-Kontext-pro', 'some-unknown-model']) {
      it(`delegates ${id} forwarding the options to the inner model and returns its result unchanged`, async () => {
        const innerResult = {
          images: ['data:image/png;base64,DELEGATED'],
          warnings: [],
          response: {},
        };
        ...
        const options = callOptions({
          providerOptions: { aihubmix: { mode: 'generate', quality: 'high' } } as any,
        });
        ...
        expect(ctorConfig.url({ path: '/images/generations' })).toBe(
