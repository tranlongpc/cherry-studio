...
const file = (byte: number): NonNullable<ImageModelV3CallOptions['files']>[number] =>
  ({ mediaType: 'image/png', data: new Uint8Array([byte]) }) as NonNullable<
    ImageModelV3CallOptions['files']
  >[number];
...
describe('Doubao (Ark) image boundary', () => {
  ...
  it('several reference images: `image` becomes an array', async () => {
    ...
    expect(req.body).toMatchObject({
      image: ['data:image/png;base64,AQ==', 'data:image/png;base64,Ag=='],
    });
  });
    ...
    const result = await runWithResponse({ data: [{ b64_json: 'AQID' }] }, (fetch) =>
      imageModel('doubao-seedream-4-0', fetch).doGenerate(
        ...
        opts({
          seed: 7,
          aspectRatio: '1:1',
          mask: file(9) as unknown as ImageModelV3CallOptions['mask'],
        }),
      ),
    );
    expect(result.images).toEqual(['AQID']);
    expect(result.warnings.map((w) => w.type === 'unsupported' && w.feature)).toEqual([
      'aspectRatio',
      'seed',
      'mask',
    ]);
