...
describe('runFormulaFiber', () => {
  it('posts the arguments as a JSON string to the formula fiber endpoint', async () => {
    const fetchMock = vi.fn(async () =>
      fiberResponse({ status: 'succeeded', context: { output: 'plain' } }),
    );
  ...
  it('prefers encrypted_output, which is what web search returns', async () => {
    const fetchMock = vi.fn(async () =>
      fiberResponse({
        status: 'succeeded',
        context: { encrypted_output: '----MOONSHOT ENCRYPTED BEGIN----x' },
      }),
    );

    await expect(
      runFormulaFiber(
        { baseURL: 'https://api.moonshot.cn/v1', apiKey: 'k', fetch: fetchMock as never },
        'f',
        'n',
        {},
      ),
  ...
  it('falls through an empty output to encrypted_output', async () => {
    ...
    await expect(
      runFormulaFiber(
        { baseURL: 'https://x/v1', apiKey: 'k', fetch: fetchMock as never },
        'f',
        'n',
        {},
      ),
...
describe('createKimiWebSearchTool', () => {
  it('returns the fiber output verbatim so the encrypted payload survives serialization', async () => {
    ...
    const output = await (kimiTool.execute as (i: unknown, o: unknown) => Promise<unknown>)(
      { query: 'q' },
      {},
    );
