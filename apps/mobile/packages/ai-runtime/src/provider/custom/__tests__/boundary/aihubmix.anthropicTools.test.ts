...
function urlsSchemaFrom(body: unknown): Record<string, unknown> {
  const tool = (
    body as { tools?: Array<{ input_schema?: { properties?: Record<string, unknown> } }> }
  ).tools?.[0];
...
describe('AiHubMix → Anthropic tool-schema boundary (patched @ai-sdk/anthropic)', () => {
  ...
  it('preserves an explicit additionalProperties:true on a nested object while defaulting others closed', async () => {
    ...
    const inputSchema = (req.body as { tools: Array<{ input_schema: Record<string, unknown> }> })
      .tools[0].input_schema;
    const params = (inputSchema.properties as Record<string, { additionalProperties?: unknown }>)
      .params;
