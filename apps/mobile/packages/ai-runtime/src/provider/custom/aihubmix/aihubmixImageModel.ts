    ...
    doGenerate(options) {
      ...
      const aspectRatio =
        ...
        normalizeAspectRatio(
          aihubmixOptions.aspectRatio ?? aihubmixOptions.aspect_ratio ?? openaiOptions.aspectRatio,
        );
      const personGeneration = normalizePersonGeneration(
        aihubmixOptions.personGeneration ??
          aihubmixOptions.person_generation ??
          openaiOptions.personGeneration,
      );
      ...
      if (
        isGeminiImage &&
        (aspectRatio || imageSize || Object.keys(existingImageConfig).length > 0)
      ) {
...
function parseOpenAIImageResults(data: any): string[] {
  ...
  for (const item of items) {
    ...
    else if (typeof item?.base64_json === 'string')
      urls.push(`data:image/png;base64,${item.base64_json}`);
  }
  return urls;
}
...
export function createAihubmixImageModel(
  ...
  const doGenerate = async (
    ...
    const aspectRatio =
      options.aspectRatio ?? (typeof bag.aspectRatio === 'string' ? bag.aspectRatio : undefined);
    ...
    if (modelId === 'V_3') {
      ...
      if (mode === 'remix') {
        ...
        if (!response.ok) {
          const message = await readErrorMessage(response, 'Image remix failed');
          throw createPaintingGenerateError('REMOTE_ERROR', { message });
        }
    ...
    if (isDefaultModel(modelId, mode)) {
      ...
      return inner.doGenerate({
        ...options,
        providerOptions: wireNameAihubmixBag(options.providerOptions),
      });
    }
