...
export class SiliconImageModel implements ImageModelV3 {
  ...
  async doGenerate(
    ...
    const bag = (providerOptions?.silicon ?? providerOptions?.openai ?? {}) as Record<
      string,
      unknown
    >;
    ...
    if (typeof seed === 'number') body.seed = seed;
      ...
      body.seed = Number(bag.seed.trim());
...
export function createSiliconImageModel(
  modelId: string,
  config: SiliconImageModelConfig,
): SiliconImageModel {
